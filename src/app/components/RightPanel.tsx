...
const getDamageColorClass = (damage: number = 0) => {
  if (damage >= 100) return "bg-red-100 border-red-500";
  if (damage >= 75) return "bg-orange-100 border-orange-500";
  if (damage >= 50) return "bg-amber-100 border-amber-500";
  if (damage >= 25) return "bg-yellow-100 border-yellow-500";
...
function RightPanel() {
  ...
  const getDamagedCount = (items: InfrastructureItem[], damageLevel?: number) => {
  ...
  if (!analysisData) {
    return (
      <div className="w-64 flex flex-col p-4 overflow-auto max-h-screen bg-white shadow-2xl rounded-l-2xl">
        <h1 className="text-2xl font-bold text-gray-700 mb-6">Оцінка пошкоджень</h1>
        <p className="text-gray-500 text-sm">Виділіть на карті зони пошкодження та проведіть аналіз для перегляду результатів.</p>
      </div>
    );
  }
          ...
          <div key={key} className="mb-6">
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="text-md font-semibold text-blue-700">{categoryNameMap[key] || key}</h2>
                ...
                {filteredItems
                  ...
                  .map((place: InfrastructureItem, index: number) => (
                    <motion.div
                      ...
                      className={`flex items-center gap-2 ${getDamageColorClass(place.damage)} hover:bg-opacity-80 transition-all mb-2 rounded-lg p-2 shadow-sm cursor-pointer border-l-4`}
                    >
                      {getIcon(key)}
